  ...
  function cargarPeliculas(peliculas) {
    ...
    for (const pelicula of peliculas) {
      divPeliculas.innerHTML += `
          <div class="pelicula"> 
          <h2>${pelicula.nombre}</h2>
          <p><a class="btnSinopsis" data-id="${pelicula.id}" href="#">Ver sinopsis</a></p>
    ...
    const botonesSinopsis = document.querySelectorAll(".btnSinopsis");
    for (const boton of botonesSinopsis) {
      boton.addEventListener("click", (event) => {
        ...
        Swal.fire({
          title: `<h2>${pelicula.nombre}</h2>`,
          icon: "info",
          html: `<div class="sinopsis">${pelicula.sinopsis}</div>`,
